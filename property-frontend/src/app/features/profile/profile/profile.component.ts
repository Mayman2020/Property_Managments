...
export class ProfileComponent implements OnInit, OnDestroy {
    ...
    this.profileService.getMyProfile().subscribe({
      next: (res) => {
        ...
        this.form.patchValue({
          fullNameAr: this.nameAr(user),
          fullNameEn: this.nameEn(user),
          phone: user?.phone ?? '',
  ...
  heroInitials(): string {
    const name = (this.form.get('fullName')?.value as string)?.trim()
      || (this.i18n.currentLang === 'ar'
        ? (this.form.get('fullNameAr')?.value as string)?.trim()
        : (this.form.get('fullNameEn')?.value as string)?.trim())
      || (this.form.get('fullNameAr')?.value as string)?.trim()
      || (this.form.get('fullNameEn')?.value as string)?.trim()
  ...
  private nameAr(user: User | undefined | null): string {
    if (!user) return '';
    return (user.fullNameAr ?? user.tenantLink?.fullNameAr ?? user.ownerLink?.fullNameAr ?? user.fullName ?? '').trim();
  }

  private nameEn(user: User | undefined | null): string {
    if (!user) return '';
    return (user.fullNameEn ?? user.tenantLink?.fullNameEn ?? user.ownerLink?.fullNameEn ?? user.fullName ?? '').trim();
  }

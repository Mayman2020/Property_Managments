...
export class NotificationsPageComponent implements OnInit {
  ...
  notificationTitle(notification: AppNotification): string {
    ...
    const singleTitle = this.i18n.pickBilingualSegment(rawTitle, 'title');

    if (rawTitle.includes(' | ')) {
      return singleTitle || this.i18n.instant('NOTIFICATIONS.GENERAL');
    }
    ...
    if (this.hasArabic(singleTitle)) return singleTitle;
    return singleTitle || this.i18n.instant('NOTIFICATIONS.GENERAL');
  }

  notificationMessage(notification: AppNotification): string {
    const fromParams = this.translateFromParams(notification.params?.bodyKey, notification.params?.vars);
    if (fromParams) return fromParams;
  ...
  private translateFromParams(key: string | undefined, vars: Record<string, unknown> | undefined): string {
    if (!key) return '';
    const resolved = this.i18n.instant(key, vars ?? undefined);

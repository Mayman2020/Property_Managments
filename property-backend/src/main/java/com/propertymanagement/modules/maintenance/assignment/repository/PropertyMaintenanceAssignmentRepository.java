...
public interface PropertyMaintenanceAssignmentRepository extends JpaRepository<PropertyMaintenanceAssignment, Long> {
    ...
    @Query("""
            SELECT a FROM PropertyMaintenanceAssignment a
            WHERE a.propertyId = :propertyId
              AND a.maintenanceProviderId = :providerId
              AND a.status IN ('ACTIVE', 'PENDING_OWNER_APPROVAL')
            """)
    Optional<PropertyMaintenanceAssignment> findOpenByPropertyAndProvider(@Param("propertyId") Long propertyId, @Param("providerId") Long providerId);
}
